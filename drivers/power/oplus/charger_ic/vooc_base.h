...
*/
#define CHGPMP_DATA_MASK	0xff
#define CHGPMP_DATA_H_SHIFT	0x08

#define VOOCPHY_DATA16_SPLIT(data_src, data_l, data_h)	\
do {				\
	data_l = (data_src) & CHGPMP_DATA_MASK;	\
	data_h = ((data_src) >> CHGPMP_DATA_H_SHIFT) & CHGPMP_DATA_MASK;	\
} while(0);
...
void (*set_pd_svooc_config)(vooc_manager *chip, bool enable);
bool (*get_pd_svooc_config)(vooc_manager *chip);
void (*dump_regs)(void);
};
...
extern int oplus_voocphy_irq_gpio_init(void);
...
extern BattMngr_err_code_e vooc_monitor_stop(unsigned long data);
extern void oplus_voocphy_set_pdsvooc_adapter_config(vooc_manager *chip, bool enable);
extern bool oplus_voocphy_get_pdsvooc_adapter_config(vooc_manager *chip);
