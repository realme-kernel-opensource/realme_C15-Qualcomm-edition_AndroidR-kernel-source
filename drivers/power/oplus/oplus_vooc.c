...
#include "charger_ic/vooc_base.h"
...
extern void opchg_voocphy_set_switch_fast_charger(void);
extern void opchg_voocphy_set_switch_normal_charger(void);
extern int oplus_voocphy_init(void);
...
//	int chg_type = 0;
	...
	if (oplus_chg_get_voocphy_support()) {
		...
		if (chg_type != POWER_SUPPLY_TYPE_USB_DCP) {
			chg_debug("is not POWER_SUPPLY_TYPE_USB_DCP \n");
			return;
		}
		...
		if (oplus_vooc_get_fastchg_to_normal()) {
			...
			if (mode == VOOC_CHARGER_MODE) {
				opchg_voocphy_set_switch_fast_charger();
				oplus_voocphy_init();
			} else if (mode == NORMAL_CHARGER_MODE) {
				oplus_chg_set_usb_mid_boost(0);
...
void fw_update_thread(struct work_struct *work)
{
	...
	int retry = 5;
	char version[10];
	bool vooc_started_before = false;
		...
		if(!ret) {
			...
			if(chip->vops->fw_check_then_recover) {
				...
				if (ret == FW_CHECK_MODE) {
					...
					vooc_started_before = chip->fastchg_started;
					chip->fastchg_dummy_started = false;
					chip->fastchg_started = false;
					chip->allow_reading = true;
					del_timer(&chip->watchdog);
...
{
	if (oplus_voocphy_get_btb_temp_over() == true) {
		return true;
	}
...
extern void oplus_reset_fastchg_after_usbout(void);
...
static bool is_allow_fast_chg_real(void)
{
	...
	allow_real = voocphy_get_real_fastchg_allow();
	if (oplus_vooc_get_fastchg_dummy_started() == true  && (!allow_real)) {
		chg_debug(" oplus_vooc_get_fastchg_dummy_started is true\n");
		cpuboost_charge_event(CPU_CHG_FREQ_STAT_AUTO);
		return false;
	}

	if (oplus_chg_get_voocphy_support() && oplus_voocphy_user_exit_fastchg()) {
		chg_debug("oplus_voocphy_user_exit_fastchg true\n");
		cpuboost_charge_event(CPU_CHG_FREQ_STAT_AUTO);
		return false;
	}
	return true;
...
void oplus_voocphy_chargerid_check(void)
{
...
void oplus_vooc_switch_fast_chg(void)
{
	if (oplus_chg_get_voocphy_support()) {
		...
		if (oplus_vooc_get_fastchg_to_normal()) {
			chg_err(" fast_switch_to_noraml is true\n");
			cpuboost_charge_event(CPU_CHG_FREQ_STAT_AUTO);
		} else {
			if (is_allow_fast_chg_real()) {
				if (voocphy_get_real_fastchg_allow() == false
						&& oplus_vooc_get_fastchg_to_warm() == true) {
					chg_err(" fastchg_allow false, to_warm true, don't switch to vooc mode\n");
				} else {
					oplus_voocphy_chargerid_check();
					oplus_basetimer_monitor_start();
...
extern int oplus_voocphy_get_switch_gpio_val(void);
