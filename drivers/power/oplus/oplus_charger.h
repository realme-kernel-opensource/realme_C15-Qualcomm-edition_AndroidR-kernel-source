...
typedef enum {
	QC_TEMP_STATUS__VIRTUAL = 0,	/*-100c~16c*/
	QC_TEMP_STATUS__COOL,	        /*16C~22C*/
	QC_TEMP_STATUS__NORMAL,		/*22C~34C*/
	QC_TEMP_STATUS__WARM,		/*34C~37C*/
	QC_TEMP_STATUS__LITTLE_HOT,	/*37C~40C*/
	QC_TEMP_STATUS__HOT,		/*40C~42C*/
	QC_TEMP_STATUS__TOO_HOT,	/*42C~45C*/
	QC_TEMP_STATUS__INVALID
}
OPLUS_CHG_QC_TEMP_STATUS;
...
typedef enum {
	...
	CHARGER_SUBTYPE_PD,
	CHARGER_SUBTYPE_QC,
	CHARGER_SUBTYPE_QC_2,
	CHARGER_SUBTYPE_QC_3,
...
struct oplus_chg_limits {
	...
	int qc_temp_hot_fastchg_ma_led_on;
	...
	bool qc_temp_little_hot_5v_support;
	int qc_temp_little_hot_decidegc;
	int qc_temp_little_too_hot_decidegc;
	...
	int qc_temp_warm_fastchg_ma_hight;
	int qc_temp_warm_fastchg_ma_low;
	#endif
	int qc_temp_little_hot_fastchg_ma;
	#ifdef OPLUS_FEATURE_CHG_BASIC
	int qc_temp_little_hot_fastchg_ma_hight;
	int qc_temp_little_hot_fastchg_ma_low;
	#endif
	int qc_temp_little_too_hot_fastchg_ma;
...
struct oplus_voocphy_chip {
	...
	unsigned long cpu0_freq;
	unsigned long cpu1_freq;
	unsigned long cpu2_freq;
	unsigned long cpu3_freq;
	int	disconn_pre_vbus;
	int	disconn_pre_ibus;
	int	disconn_pre_vbat;
	int	disconn_pre_ibat;
	int	disconn_pre_vbat_calc;
	int disconn_pre_charger_type;
	int	voocphy_enable;
	int	vbus_adjust_cnt;
	int irq_rcvok_num;
...
/*	struct battery_data battery_main	*/
	...
	bool fg_bcl_poll;
	bool chg_powersave;
	bool qc_v3p5_spec_support;
	bool qc_chg_tbat_feature;
	bool rum_chg_thermal_feature;
	bool healthd_ready;
	int cv_soc;
#ifdef CONFIG_FB
	...
	bool soda_rum_support;
	bool qc_lower_fcc_noshake_support;
