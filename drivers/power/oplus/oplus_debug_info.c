...
enum oplus_chg_debug_info_notify_type {
	...
	OPLUS_CHG_DEBUG_NOTIFY_TYPE_VOOC_BREAK,
...
enum oplus_chg_debug_info_notify_flag {
	...
	OPLUS_NOTIFY_CHG_SLOW_VOOC_BREAK,
	OPLUS_NOTIFY_CHG_SLOW_GENERAL_BREAK,
...
static struct oplus_chg_debug_notify_policy oplus_chg_debug_notify_policy[] = {
	...
	[OPLUS_NOTIFY_CHG_SLOW_VOOC_NON_START]               = {OPLUS_CHG_NOTIFY_ONCE,         1,  1,   -1},
	...
	[OPLUS_NOTIFY_CHG_SLOW_VOOC_BREAK]      			= {OPLUS_CHG_NOTIFY_ONCE,         1,    1,      -1},
	[OPLUS_NOTIFY_CHG_SLOW_GENERAL_BREAK]      			= {OPLUS_CHG_NOTIFY_ONCE,         1,    1,      -1},
};
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
...
#endif
	{
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				...
				"OTHER[ %d %d %d %d %d %d ], "
				"VOOCPHY[%ld, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d], ",
				...
				chip->batt_exist, chip->batt_full, chip->chging_on, chip->in_rechging, chip->charging_state, oplus_chg_debug_info.total_time,
				chip->tbatt_temp, chip->batt_volt, chip->batt_volt_min, chip->icharging, chip->ibus, chip->soc, chip->ui_soc, chip->soc_load, chip->batt_rm, chip->batt_fcc,
				...
				chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode, chip->chargerid_volt, chip->chargerid_volt_got,
				chip->voocphy.cpu0_freq, chip->voocphy.cpu1_freq, chip->voocphy.cpu2_freq, chip->voocphy.cpu3_freq, chip->voocphy.ap_disconn_issue,
				chip->voocphy.disconn_pre_ibat, chip->voocphy.disconn_pre_ibus, chip->voocphy.disconn_pre_vbat, chip->voocphy.disconn_pre_vbat_calc, chip->voocphy.disconn_pre_vbus, 
				chip->voocphy.r_state, chip->voocphy.vbus_adjust_cnt, chip->voocphy.voocphy_cp_irq_flag, chip->voocphy.voocphy_enable, chip->voocphy.voocphy_iic_err,
				chip->voocphy.voocphy_vooc_irq_flag,
				chip->voocphy.irq_rcvok_num, chip->voocphy.ap_handle_timeout_num, chip->voocphy.rcv_date_err_num, chip->voocphy.rcv_done_200ms_timeout_num);
		{
			int i;
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					", VOOCPHY_REG[");
		...
		}
		{
			if(chip->gauge_iic_err_time_record != 0) {
				ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
						", gauge_iic_err_%d[%d]", chip->gauge_iic_err, chip->gauge_iic_err_time_record);
	...
	if(chip->detect_detach_unexpeactly == 1) {
		opchg_voocphy_clear_dbg_info();
		oplus_chg_debug_info.chg_cnt[OPLUS_NOTIFY_CHG_SLOW_VOOC_BREAK] = 0;
		oplus_chg_debug_info.notify_flag &= (~(1L << OPLUS_NOTIFY_CHG_SLOW_VOOC_BREAK));
		oplus_chg_debug_info.notify_type &= (~(1 << OPLUS_CHG_DEBUG_NOTIFY_TYPE_VOOC_BREAK));
	}
...
static int oplus_chg_fastchg_is_allowed(struct oplus_chg_chip *chip)
{
	...
	if (vooc_chip) {
		...
		return 1;
	} else if (oplus_chg_get_voocphy_support() == true) {
		if (voocphy_get_real_fastchg_allow() == true) {
			return 1;
		} else {
			return 0;
		}
...
static int oplus_chg_break_check(struct oplus_chg_chip *chip)
{
	if (chip == NULL)
		return 0;
	if(chip->detect_detach_unexpeactly == 1){
		oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_VOOC_BREAK);
		oplus_chg_set_notify_type(OPLUS_CHG_DEBUG_NOTIFY_TYPE_VOOC_BREAK);
	}else if(chip->detect_detach_unexpeactly == 2) {
		oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_GENERAL_BREAK);
	}
...
static int oplus_chg_chg_slow_check(struct oplus_chg_chip *chip)
{
	...
	if (opchg_get_vooc_start_fg() == false) {
		goto oplus_chg_slow_check_end;
	}
