...
static int bq27541_get_battery_mvolts(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		...
		if(gauge_ic->batt_bq28z610) {
			...
			gauge_ic->max_vol_pre = gauge_ic->batt_cell_max_vol;
			gauge_ic->min_vol_pre = gauge_ic->batt_cell_min_vol;
			if (gauge_ic->device_type_err) {
				gauge_ic->batt_vol_pre = 3800;
				dev_err(gauge_ic->dev, "device_type_err return batt_vol:%d\n", gauge_ic->batt_vol_pre);
				return gauge_ic->batt_vol_pre;
			}
			return volt;
		}
...
cb_exit:
	...
	if (gauge_ic->device_type_err) {
		dev_err(gauge_ic->dev, "device_type_err return temperature:%d\n", 250);
		return 250;
	}
...
read_soc_err:
	if (gauge_ic->device_type_err && ret == 0) {
		dev_err(gauge_ic->dev, "device_type_err soc_pre:%d\n", gauge_ic->soc_pre);
		if (gauge_ic->soc_pre >= 0 && gauge_ic->soc_pre <= 100) {
			return gauge_ic->soc_pre;
		} else {
			return 50;
		}
...
static int bq27541_get_average_current(void)
{
	...
	if (gauge_ic->device_type_err) {
		gauge_ic->current_pre = 100;
		dev_err(gauge_ic->dev, "device_type_err return current:%d\n", gauge_ic->current_pre);
		return gauge_ic->current_pre;
	}
...
#define OPLUS_GAUGE_CHECK_INTERVAL_SEC 		5
#define OPLUS_GAUGE_CHECK_INTERVAL	round_jiffies_relative(msecs_to_jiffies(OPLUS_GAUGE_CHECK_INTERVAL_SEC*1000))
static void bq27541_hw_config(struct chip_bq27541 *chip);
static int seal(void);
static void bq27541_err_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct chip_bq27541 *chip = container_of(dwork, struct chip_bq27541, err_check_wk);
	int ret = 0;
	int device_type = 0;

	if (chip->device_type_err) {
		ret = bq27541_read_i2c(BQ27541_BQ27411_REG_CNTL, &device_type);
		udelay(66);
		if (ret == 0) {
			bq27541_hw_config(chip);
		}
	}

	/* run again after interval */
	if (chip->device_type_err) {
		pr_err("device_type_err2");
		schedule_delayed_work(&chip->err_check_wk, OPLUS_GAUGE_CHECK_INTERVAL);
	} else {
		seal();
	}
}
...
static void bq27541_hw_config(struct chip_bq27541 *chip)
{
	...
	ret = bq27541_read_i2c(BQ27541_BQ27411_REG_CNTL, &device_type);
	if (ret != 0) {
		chip->device_type_err = true;
		device_type = DEVICE_TYPE_BQ27411;
		pr_err("device_type_err1");
		schedule_delayed_work(&chip->err_check_wk, OPLUS_GAUGE_CHECK_INTERVAL);
	} else {
		chip->device_type_err = false;
	}
	udelay(66);
	bq27541_cntl_cmd(BQ27541_BQ27411_SUBCMD_CTNL_STATUS);
	udelay(66);
		...
		const struct i2c_device_id *id)
{
	...
	/*add check device type mornitor work*/
	fg_ic->device_type_err = false;
	INIT_DELAYED_WORK(&fg_ic->err_check_wk, bq27541_err_check_work);
rerun :
	rerun_num--;
